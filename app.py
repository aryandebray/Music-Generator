...
def save_audio(samples: torch.Tensor,text):
    sample_rate=32000
    ...
    for idx,audio in enumerate(samples):
        s=text.replace(" ","_")
        audio_path=os.path.join(save_path,f"{s}.wav")
...
def main():
    ...
    if text and time_slider:
        text_display="Your Description: "+""+text
        st.write(text_display)
        time_display="Time Duration of clip: "+""+str(time_slider)
        st.write(time_display)
        st.subheader("Generated Music")

        music_tensors=generate_music_tensors(text,time_slider)
        print("Music Tensors: ",music_tensors)
        save_music_file=save_audio(music_tensors,text)
        audio_filepath='AudioOutput/'+text.replace(" ","_")+'.wav'
